...
describe('Тесты сайта кинотеатра', () => {
  ...
  it('Тестирование даты сеанса, выбора фильма и бронирования уже купленого места', () => {
    cy.get('.page-nav__day:nth-of-type(2)').click()
    cy.get('.movie').eq(1).contains('11:00').click()
       seats.forEach((seat) => {
        cy.get(`:nth-child(${seat.row}) > :nth-child(${seat.seat})`).click()
    })
    cy.get('.acceptin-button').click()
